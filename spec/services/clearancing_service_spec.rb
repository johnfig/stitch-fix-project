...
describe ClearancingService do
  ...
  describe '#process_text_field' do
    context "total success" do
      let(:item_1) { FactoryGirl.create(:item) }
      let(:item_2) { FactoryGirl.create(:item) }
      let(:string) { "#{item_1.id},#{item_2.id}" }

      before do
        @clearancing_status = clearancing_service.process_text_field(string)
      end
      ...
      it "adds all the items to the batch" do
        expect(@clearancing_status.clearance_batch.items.pluck(:id).sort).to eq([item_1.id,item_2.id])
      end
    ...
    context "partial success" do
      let(:valid_item)       { FactoryGirl.create(:item, status: 'sellable') }
      let(:unsellable_item)   { FactoryGirl.create(:item, status: 'clearanced') }
      let(:non_existent_id)   { 987654 }
      let(:invalid_id)        { 'no thanks' }
      let(:no_id)             { nil }
      ...
      let(:string) { [valid_item.id,invalid_items].flatten.join(',') }
      ...
      it "includes all valid items in the batch" do
        expect(@clearancing_status.clearance_batch.items.pluck(:id)).to eq([valid_item.id])
      end
    end
    ...
    context 'error states' do
      ...
      let(:no_id)            { nil }
      let(:float_id)         { 123.45 }
      let(:string) { "#{non_existent_id},#{invalid_id},#{no_id},#{float_id},#{unsellable_item.id}" }
      before do
        @clearancing_status = clearancing_service.process_text_field(string)
      end

      it "detects all errors generated by invalid items" do
        expect(@clearancing_status.errors.count).to eq(string.split(',').count)
        [ invalid_id, no_id ].each do |bad_id|
          expect(@clearancing_status.errors).to include("Item id #{bad_id.to_i} is not valid")
        end
        expect(@clearancing_status.errors).to include("Item id #{non_existent_id} could not be found")
